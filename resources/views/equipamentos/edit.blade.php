...
<div class="card">
    <div class="card-header">
        <h1>Editar Equipamento: {{ $equipamento->hostname }}</h1>
    </div>
    <div class="card-body">
        <form action="/equipamentos/{{ $equipamento->id }}" method="POST">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('hostname') is-invalid @enderror" 
                               id="hostname" name="hostname" value="{{ old('hostname', $equipamento->hostname) }}">
                        @error('hostname')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('ip') is-invalid @enderror" 
                               id="ip" name="ip" value="{{ old('ip', $equipamento->ip) }}">
                        @error('ip')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('model') is-invalid @enderror" id="model" name="model">
                            @foreach(App\Models\Equipamento::model as $model)
                                <option value="{{ $model }}" {{ old('model', $equipamento->model) == $model ? 'selected' : '' }}>
                                    {{ $model }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('poe_type') is-invalid @enderror" id="poe_type" name="poe_type">
                            <option value="poe" {{ old('poe_type', $equipamento->poe_type) == 'poe' ? 'selected' : '' }}>POE</option>
                            <option value="poe+" {{ old('poe_type', $equipamento->poe_type) == 'poe+' ? 'selected' : '' }}>POE+</option>
                            <option value="none" {{ old('poe_type', $equipamento->poe_type) == 'none' ? 'selected' : '' }}>Não POE</option>
                        </select>
                        @error('poe_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('qtde_portas') is-invalid @enderror" 
                               id="qtde_portas" name="qtde_portas" min="1" 
                               value="{{ old('qtde_portas', $equipamento->qtde_portas) }}">
                        @error('qtde_portas')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('patrimonio') is-invalid @enderror" 
                               id="patrimonio" name="patrimonio" value="{{ old('patrimonio', $equipamento->patrimonio) }}">
                        @error('patrimonio')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select class="form-select @error('predio_id') is-invalid @enderror" id="predio_id" name="predio_id">
                            ...
                            @foreach($predios as $predio)
                                <option value="{{ $predio->id }}" 
                                    {{ old('predio_id', $equipamento->predio_id) == $predio->id ? 'selected' : '' }}>
                                    {{ $predio->nome }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="d-flex justify-content-between">
                ...
                <a href="/equipamentos/{{ $equipamento->id }}" class="btn btn-secondary">Cancelar</a>
            </div>
        </form>
    </div>
</div>
